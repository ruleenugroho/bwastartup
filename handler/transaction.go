...
func (h *transactionHandler) GetCampaignTransactions(c *gin.Context) {
	...
	if err != nil {
		response := helper.APIResponse("Failed to get campaign's transactions2", http.StatusBadRequest, "error", err.Error())
		c.JSON(http.StatusBadRequest, response)
		return
	}
...
func (h *transactionHandler) CreateTransaction(c *gin.Context) {
	...
	response := helper.APIResponse("Transaction successfully created", http.StatusOK, "success", transaction.FormatTransaction(NewTransaction))
	c.JSON(http.StatusOK, response)
}
